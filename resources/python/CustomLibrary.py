...
class CustomLibrary:
    ...
    def __init__(self, tc_session_reset=True) -> None:
        ...
        self.login = ''
        self.password = ''
        self._connection: Optional[TestObject] = None

    def connect(self, ip):
        sleep(1)  # Simulate some connection delay
    ...
    def change_users_name(self, login, new_name):
        '''Changes the name of a user by its name.
        Requires Admin privileges!'''
        user_id = self.get_user_id(login)
        self.connection.put_user_name(self.session, new_name, user_id=user_id)
    ...
    def get_connection_details(self):
        if self.connection and hasattr(self.connection, 'ip'):
            return f'Connected to server at IP: {self.connection.ip}'
        else:
            return 'No valid connection'

    def get_user_dictionary(self, user_id):
        return self.connection.get_user_data(self.session, user_id)